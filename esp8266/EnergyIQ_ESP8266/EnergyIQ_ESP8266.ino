 ...
 * Local dashboard + push readings to MongoDB
 */
...
// ---- Backend (EnergyIQ) ----
const char* BACKEND_URL = "https://energyiqm.onrender.com";  // Your Node.js API URL
...
HTTPClient http;
  ...
  http.end();
  if (!lastPushOk) Serial.printf("Push %d\n", code);
}
...
<div class="container">
  ...
  <p>Live readings from PZEM-004T and relay control • Synced to EnergyIQ</p>
  ...
  <div class="footer">
    ESP8266 IP: <span id="ip">%IP%</span> | Auto refresh: 2s | Backend push: 5s
  </div>
</div>
<script>
  ...
  fetch('/readings')
    ...
    .then(data => {
      ...
      if (data.ok) {
        ...
        badge.className = 'badge ' + (data.backend ? 'badge-ok' : 'badge-fail');
      } else {
...
void loop() {
  ...
  if (millis() - lastPush >= PUSH_INTERVAL_MS) {
    ...
    // Skip push when any reading is NaN to avoid invalid JSON (nan)
    if (!isnan(v) && !isnan(i) && !isnan(p) && !isnan(e) && !isnan(f)) {
