...
const BillPrediction = () => {
  const [units, setUnits] = useState(380);
  const { breakdown, total } = useMemo(() => calculateTNEBBill(units), [units]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-xl p-6 shadow-card border border-border">
        <label className="text-sm font-medium text-card-foreground block mb-2">Units Consumed (kWh)</label>
        <input
          type="range" min={0} max={1000} value={units} onChange={(e) => setUnits(Number(e.target.value))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl p-6 shadow-card border border-border">
          ...
          <div className="space-y-3">
            {breakdown.map((item) => (
              <div key={item.slab} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                <div>
                  <p className="text-sm font-medium text-card-foreground">{item.slab}</p>
                  <p className="text-xs text-muted-foreground">{item.units} units × ₹{item.rate}</p>
                </div>
                <span className={`text-sm font-mono font-semibold ${item.rate === 0 ? "text-energy-green" : "text-card-foreground"}`}>
        ...
        <div className="space-y-4">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl p-6 shadow-card border border-border gradient-primary">
